...
import type { ThreatCategoryStatus } from '@/types'
...
import { canUseProFeature, type BillingStatus } from '@/lib/billing'
...
export interface SecurityReportViewProps {
...
/**
 * Security Report — extracted from ReportView.tsx (2026-09-04) for architectural consistency with
 * the other five report views, with EVERY existing behavior preserved verbatim: share settings
 * (Firestore read/write), ScanComparison (live Firestore-backed), the verification badge, and
 * every existing content section. Nothing here was redesigned — only moved and given a real "where
 * can this agent hurt us?" framing at the top, built ONLY from evidence this component already
 * receives (capability chains, security categories, findings) — never a new, invented attack path.
 */
export function SecurityReportView({ data, report, reportId, user = null, isOwner = false, onReportUpdate, onNewScan, reportUrl, billingStatus }: SecurityReportViewProps) {
  const [copied, setCopied] = useState<string | null>(null)
  const [showShare, setShowShare] = useState(false)
  const [saving, setSaving] = useState(false)
  const [saved, setSaved] = useState(false)
  const [shareError, setShareError] = useState<string | null>(null)
  const [shareSettings, setShareSettings] = useState({ isPublic: report?.isPublic === true })
  ...
  const {
    reportId: normalizedReportId, verified, riskScore: normalizedRiskScore,
    findings: safeFindings, findingCount, criticalCount: critical, highCount: high, mediumCount: medium,
    ...
    reportInsights, evidenceFindings, complianceTags, publicReportUrl, artifactHash,
  ...
  const summary = generateSummary({
    ...
    metadata: {
      ...
      selectedPlatform: data.platform ?? normalizedBom.detectedPlatform,
  ...
  const showBadge = data.verdict === 'VERIFIED' && badgeReportId.startsWith('REPORT-')
  const badgeVersion = encodeURIComponent(`${badgeReportId}-${normalizedRiskScore}-${data.scannedAt}`)

  // "Where can this agent hurt us?" quick facts — every value below is read directly from
  // evidence this component already has, never a newly invented signal or attack path.
  const secretExposureFindings = safeFindings.filter(f => f.securityCategory === 'secrets')
  const authorizationGap = normalizedSecurityCategories.find(c => c.id === 'execution-authorization' && (c.status === 'critical' || c.status === 'needs_attention'))
  const humanOversightGap = normalizedSecurityCategories.find(c => c.id === 'human-oversight' && (c.status === 'critical' || c.status === 'needs_attention'))
  const externalSideEffectCapabilities = normalizedCapabilities.filter(c => /api|network|http|external|send|transfer|deploy|payment|email|message/i.test(c.label))
  const topAttackPaths = normalizedCapabilityChains.slice(0, 3) // real, evidence-based capability combinations — never fabricated
  ...
  const saveShareSettings = async () => {
    ...
    const updates = { isPublic: shareSettings.isPublic }
  ...
  return (
    <>
      <section style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }} className="print-border mb-6 rounded-3xl p-6 shadow-xl shadow-black/5">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--accent-purple-text)]">Where can this agent hurt us?</p>
        <h2 style={{ color: 'var(--text-primary)' }} className="mt-1 text-lg font-semibold">Real evidence only — nothing below is inferred beyond what this scan found</h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }} className="rounded-2xl p-4">
            <p style={{ color: 'var(--text-muted)' }} className="text-[11px] font-semibold uppercase tracking-wider">Credential exposure</p>
            <p style={{ color: secretExposureFindings.length > 0 ? 'var(--accent-red-text)' : 'var(--accent-green-text)' }} className="mt-1 text-sm font-semibold">{secretExposureFindings.length > 0 ? `${secretExposureFindings.length} finding${secretExposureFindings.length === 1 ? '' : 's'}` : 'None detected'}</p>
          </div>
          <div style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }} className="rounded-2xl p-4">
            <p style={{ color: 'var(--text-muted)' }} className="text-[11px] font-semibold uppercase tracking-wider">Authorization gaps</p>
            <p style={{ color: authorizationGap ? 'var(--accent-red-text)' : 'var(--accent-green-text)' }} className="mt-1 text-sm font-semibold">{authorizationGap ? authorizationGap.status === 'critical' ? 'Critical' : 'Needs attention' : 'None detected'}</p>
          </div>
          <div style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }} className="rounded-2xl p-4">
            <p style={{ color: 'var(--text-muted)' }} className="text-[11px] font-semibold uppercase tracking-wider">Human approval gaps</p>
            <p style={{ color: humanOversightGap ? 'var(--accent-red-text)' : 'var(--accent-green-text)' }} className="mt-1 text-sm font-semibold">{humanOversightGap ? humanOversightGap.status === 'critical' ? 'Critical' : 'Needs attention' : 'None detected'}</p>
          </div>
          <div style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }} className="rounded-2xl p-4">
            <p style={{ color: 'var(--text-muted)' }} className="text-[11px] font-semibold uppercase tracking-wider">External side effects</p>
            <p style={{ color: 'var(--text-primary)' }} className="mt-1 text-sm font-semibold">{externalSideEffectCapabilities.length > 0 ? `${externalSideEffectCapabilities.length} capabilit${externalSideEffectCapabilities.length === 1 ? 'y' : 'ies'}` : 'None detected'}</p>
          </div>
        </div>
        {topAttackPaths.length > 0 && (
          <div className="mt-4">
            <p style={{ color: 'var(--text-primary)' }} className="text-sm font-semibold">Top attack paths — real capability combinations this scan detected</p>
            <ol className="mt-2 space-y-1.5">
              {topAttackPaths.map((chain, i) => (
                <li key={chain.id} style={{ color: 'var(--text-secondary)' }} className="text-sm">{i + 1}. {chain.title} — <span style={{ color: 'var(--text-muted)' }} className="text-xs">{chain.impact}</span></li>
              ))}
            </ol>
          </div>
        )}
        ...
        <div className="no-print mb-6">
          ...
          {showShare && (
            <div style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }} className="mb-6 rounded-xl p-5">
              ...
              {shareSettings.isPublic && artifactHash && (
                <div className="mb-5">
                  ...
                  <div className="relative">
                    <input readOnly aria-label="Public verification link" value={`https://aimodularity.com/agentverify/verify/?hash=${artifactHash}`} style={{ backgroundColor: 'var(--input-bg)', border: '1px solid var(--input-border)', color: 'var(--text-muted)' }} className="w-full rounded-lg px-4 py-2.5 pr-20 font-mono text-xs outline-none" />
                    <button onClick={() => copyText(`https://aimodularity.com/agentverify/verify/?hash=${artifactHash}`, 'verify-link')} style={{ backgroundColor: copied === 'verify-link' ? '#7C3AED' : 'var(--card)', color: copied === 'verify-link' ? '#060A0F' : 'var(--text-muted)', border: '1px solid var(--border)' }} className="absolute right-2 top-1.5 rounded px-2 py-1 text-xs font-medium transition-all">
      ...
      <section style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }} className="print-border mb-6 rounded-xl p-6">
        ...
        <div style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }} className="rounded-xl p-5">
          ...
          <ul className="mb-4 space-y-1.5">
            {summary.bullets.map((bullet: string, i: number) => <li key={i} style={{ color: 'var(--text-secondary)' }} className="flex items-start gap-2 text-sm"><span className={`mt-1 h-1.5 w-1.5 shrink-0 rounded-full ${verified ? 'bg-[#00B37E]' : 'bg-[#E03E3E]'}`} />{bullet}</li>)}
          </ul>
