...
//
// Strict RFC 8785 (JSON Canonicalization Scheme). "Strict" means it REJECTS what the existing canonicalizeForHash
// coerces: undefined becomes null, unknown objects become String(x), and NaN passes through JSON.stringify as null.
// A canonicalizer that coerces lets two different in-memory values share one digest.
//
...
import { isWellFormedString, MAX_DEPTH, parseStrictJson } from './strictJson.mjs'
...
export function canonicalize(value, { maxDepth = MAX_DEPTH, admitNumber } = {}) {
  const stack = new Set()

  function ser(v, depth, where) {
    if (depth > maxDepth) throw new JcsError('JCS_TOO_DEEP', `nesting deeper than ${maxDepth} at ${where}`)
...
export const canonicalizeSigned = value => canonicalize(value, { admitNumber: numberProblem })
export const signedBytes = value => new TextEncoder().encode(canonicalizeSigned(value))
