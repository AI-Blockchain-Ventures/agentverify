...
const D = loadVector('domains.v2.json')
...
test('every signed vector\'s signing input begins with exactly the registered tag for its type, and every key-set purpose is registered', () => {
  ...
  const inSets = new Set(Object.values(BUNDLES.keySets).flatMap(k => k.keys.map(e => e.purpose)).filter(Boolean))
  assert.ok(purposes.has('agentverify-profile-v1'))
  for (const p of inSets) assert.ok(purposes.has(p) || p === 'agentverify-scan-v1', `unregistered purpose ${p}`)
