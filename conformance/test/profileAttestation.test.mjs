...
import { applyPatch, assertSubset, caseOptions, loadVector, resolveSource } from './helpers.mjs'

const BUNDLES = loadVector('bundles.v2.json')
const SIGNING = loadVector('signing.v2.json')
const DIGESTS = loadVector('assessment-digest.v2.json')
const KEYIDS = loadVector('keyid.v2.json')
const AVPKG = loadVector('avpkg.v2.json')
...
const runCase = c => ref.verifyProfileBundle(resolveSource(BUNDLES, c.source), caseOptions(BUNDLES, c))
...
test('coverage: every result a verifier can report appears in at least one vector', () => {
  ...
  assert.deepEqual([...interp].sort(), ['SUPPORTED', 'UNSUPPORTED_PROFILE', 'UNSUPPORTED_PROFILE_VERSION', 'UNSUPPORTED_SCHEMA'])
...
test('there is no TRUSTED key state anywhere: not in the reference, not in any vector', () => {
  ...
  const vector = JSON.parse(readFileSync(path.join(here, '..', 'vectors', 'bundles.v2.json'), 'utf8'))
  const allowed = vector.cases.find(c => c.name === 'policy.invalid-unknown-state')
  assert.deepEqual(allowed.policy.acceptedKeyStates, ['TRUSTED'], 'the one vector that names TRUSTED does so to prove it is not a valid state')
  assert.doesNotMatch(JSON.stringify({ ...vector, cases: vector.cases.filter(c => c !== allowed && c.name !== 'key-state.no-key-set') }), /"keyTrust"|TRUSTED/)
})
...
test('CROSS-CHECK REGISTRY: exported by the reference, frozen in the vectors, and every entry has a "signer lied" vector', () => {
  ...
  assert.deepEqual(BUNDLES.crossChecks, registry, 'the frozen list is exactly the reference registry, in order')
...
for (const name of ['B1', 'B2']) {
  test(`FORMAT-PRESERVING matrix: every signed payload field of ${name}, changed to another well-formed value, is stopped by the SIGNATURE`, async () => {
    ...
    for (const leaf of leafPaths(bundle.attestation.payload)) {
      ...
      if (rel in DECIDED_BEFORE_SIGNATURE) {
        ...
        assert.equal((await ref.verifyProfileBundle(mutated)).integrity, DECIDED_BEFORE_SIGNATURE[rel], rel)
        continue
      }
      ...
      const r = await ref.verifyProfileBundle(mutated)
  ...
  test(`exhaustive: removing ANY single member from ${name} is never VALID`, async () => {
    const bundle = signedBundle(name)
    for (const p of leafPaths(bundle)) assert.notEqual((await ref.verifyProfileBundle(applyPatch(bundle, [{ op: 'remove', path: p }]))).integrity, 'VALID', `removing ${p} still verified`)
  })
  ...
  test(`exhaustive: adding an unknown member to ANY object in ${name} is never VALID`, async () => {
    ...
    walk(bundle, '')
    for (const p of objects) assert.notEqual((await ref.verifyProfileBundle(applyPatch(bundle, [{ op: 'add', path: `${p}/zzUnknown`, value: 'x' }]))).integrity, 'VALID', `an unknown member at ${p || '(root)'} still verified`)
  })
  ...
  test(`exhaustive: swapping any two items of any array in ${name}'s assessment is never VALID (order is signed)`, async () => {
    ...
    for (const [p, arr] of arrays) for (let i = 0; i < arr.length; i++) for (let j = i + 1; j < arr.length; j++) {
      ...
      swaps++
      assert.equal((await ref.verifyProfileBundle(applyPatch(bundle, [{ op: 'swap', path: p, i, j }]))).integrity, 'BINDING_MISMATCH', `swapping ${i}<->${j} in ${p} still verified`)
    }
...
test('key order in transport does not matter but array order does', async () => {
  ...
  assert.equal((await ref.verifyProfileBundle(BUNDLES.cases.find(c => c.name === 'equivalent.object-keys-reordered').source.text)).integrity, 'VALID')
...
test('D10: changing ONLY one interpretation version changes the assessment digest, the signing input and the signature, and nothing else', async () => {
  ...
  for (const d of BUNDLES.d10) {
    ...
    const verify = (bundle, input) => subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, key, Buffer.from(bundle.attestation.signature, 'base64'), input)
    assert.equal(await verify(base, ref.signingInput(base.attestation.payload)), true)
    assert.equal(await verify(variant, ref.signingInput(variant.attestation.payload)), true)
    assert.equal(await verify(base, ref.signingInput(variant.attestation.payload)), false, `${d.field}: the ORIGINAL signature does not cover the changed version`)
    assert.equal(await verify(variant, ref.signingInput(base.attestation.payload)), false, `${d.field}: the NEW signature does not cover the original version`)
    assert.equal((await ref.verifyProfileBundle(base)).integrity, 'VALID')
    assert.equal((await ref.verifyProfileBundle(variant)).integrity, 'VALID', `${d.field}: integrity is VALID even when the profile version is one this verifier cannot interpret`)
  }
})
...
test('bundleVersion is independent unsigned framing: not in the payload, not in any digest, not in any signing input', async () => {
  ...
  assert.equal((await ref.verifyProfileBundle(b)).integrity, 'UNSUPPORTED_BUNDLE_VERSION', 'refused, and NOT reported as a signature problem')
  const supported = await ref.verifyProfileBundle(signedBundle('B1'), { supportedBundleVersions: ['1.0.0', '9.9.9'] })
...
test('a verifier can widen its supported bundle versions without touching attestation support, and vice versa', async () => {
  ...
  assert.equal((await ref.verifyProfileBundle(b, { supportedBundleVersions: ['1.0.0', '1.1.0'] })).integrity, 'VALID')
  const p = signedBundle('S_version-1.1.0')
  assert.equal((await ref.verifyProfileBundle(p, { supportedBundleVersions: ['1.0.0', '1.1.0'] })).integrity, 'UNSUPPORTED_VERSION')
})
...
test('offline and portable: verification needs no workspace, no network and no key set', async () => {
  ...
  try {
    const r = await ref.verifyProfileBundle(signedBundle('B2'))
...
test('key purpose is a property of the KEY SET entry, never of the attestation', async () => {
  ...
  assert.equal((await ref.verifyProfileBundle(b)).integrity, 'MALFORMED')
  const p = signedBundle('B1'); p.attestation.payload.keyPurpose = ref.PROFILE_KEY_PURPOSE
  assert.equal((await ref.verifyProfileBundle(p)).integrity, 'MALFORMED')
})
...
test('ROLLBACK: retained state is keyed by issuer, and the bare-number option no longer exists', async () => {
  await assert.rejects(() => ref.verifyProfileBundle(signedBundle('B1'), { keySet: BUNDLES.keySets.active, highestAcceptedKeySetSequence: 5 }), e => e instanceof TypeError && /keyed by issuer/.test(e.message))
  ...
  assert.equal((src.match(/highestAcceptedKeySetSequence/g) ?? []).length, 3, 'the guard, its message and the comment explaining it')
})
...
test('ROLLBACK: issuer A\'s retained sequence cannot affect issuer B, and B\'s own state applies to B', async () => {
  ...
  const a = await ref.verifyProfileBundle(signedBundle('B1'), { keySet: BUNDLES.keySets.sequence3, retainedSequenceByIssuer: { [ISSUER_B]: 1_000_000 } })
...
test('ROLLBACK: the issuer is taken from the bundle\'s OWN payload; a key set for another issuer consults no state at all', async () => {
  ...
  const r = await ref.verifyProfileBundle(signedBundle('B1'), { keySet: BUNDLES.keySets.issuerMismatch, retainedSequenceByIssuer: lookup })
  ...
  const s = await ref.verifyProfileBundle(signedBundle('B1'), { keySet: BUNDLES.keySets.sequence3, retainedSequenceByIssuer: lookup })
  ...
  const m = await ref.verifyProfileBundle(signedBundle('B_issuerOther'), { keySet: BUNDLES.keySets.otherIssuer, retainedSequenceByIssuer: new Map([[ISSUER_A, 99], [ISSUER_B, 1]]) })
...
test('ROLLBACK WORDING: sequence metadata is rollback DETECTION support that needs retained issuer state; it is not freshness or protection', async () => {
  ...
  assert.equal((await ref.verifyProfileBundle(signedBundle('B1'), { keySet: forged, retainedSequenceByIssuer: { [ISSUER_A]: 5 } })).keyState, 'KEY_ACTIVE')
  assert.equal((await ref.verifyProfileBundle(signedBundle('B1'), { keySet: forged })).keySetSequenceCheck, 'NO_RETAINED_STATE')
  for (const generatedAt of ['1999-01-01T00:00:00.000Z', '2999-01-01T00:00:00.000Z']) {
    ...
    assert.equal((await ref.verifyProfileBundle(signedBundle('B1'), { keySet: odd })).keyState, 'KEY_ACTIVE', 'generatedAt is informational: never evaluated')
  }
  const src = readFileSync(path.join(here, '..', 'reference', 'profileAttestation.mjs'), 'utf8')
  assert.match(src, /rollback DETECTION support that works only when the verifier actually retains prior issuer state/)
  assert.match(src, /not rollback[\s*]*PROTECTION and not a freshness guarantee/)
...
test('NO KEY SET: integrity can be VALID, the key state is NOT_EVALUATED, and NO policy is satisfied. Nothing becomes a trust conclusion', async () => {
  for (const name of ['B1', 'B2']) {
    const bare = await ref.verifyProfileBundle(signedBundle(name))
    ...
    for (const policy of [{}, { acceptedKeyStates: ['KEY_ACTIVE', 'KEY_RETIRED'] }, { requiredProfile: 'owasp-agentic-skills-2026' }]) {
      const r = await ref.verifyProfileBundle(signedBundle(name), { policy })
...
test('every key-set entry\'s purpose comes from the domain registry, and no signature path reads a purpose from the attestation', () => {
  for (const ks of Object.values(BUNDLES.keySets)) for (const e of ks.keys) if (e.purpose) assert.ok([ref.PROFILE_KEY_PURPOSE, 'agentverify-scan-v1'].includes(e.purpose))
})
...
test('LIVE low-S proof with a throwaway in-memory key: WebCrypto emits both forms, the verifier accepts only low-S, and the high-S twin is a mathematically VALID signature', async () => {
  ...
  for (let i = 0; i < 40 && !(sawHigh && sawLow); i++) {
    ...
    assert.equal((await ref.verifyProfileBundle(low)).integrity, 'VALID')
    const h = await ref.verifyProfileBundle(high)
...
test('the profile registry is CLOSED, own-key only, and interpretation never turns into a signature failure', async () => {
  assert.deepEqual(Object.keys(ref.SUPPORTED_PROFILES), ['owasp-agentic-skills-2026'])
  assert.ok(Object.isFrozen(ref.SUPPORTED_PROFILES))
  for (const id of ['constructor', 'toString', '__proto__', 'hasOwnProperty', 'valueOf']) assert.equal(Object.hasOwn(ref.SUPPORTED_PROFILES, id), false, id)
  for (const n of ['interpretation.unknown-profile', 'interpretation.unknown-profile-version', 'interpretation.framework-not-registered', 'interpretation.registry-is-closed', 'interpretation.future-schema', 'interpretation.schema-decided-before-profile']) {
...
test('bundle text over the ceiling is refused BEFORE parsing (no parse work is done on it)', async () => {
  assert.equal(ref.MAX_BUNDLE_TEXT_BYTES, 16 * 1024 * 1024)
  const huge = `{${'"a":1,'.repeat(3 * 1024 * 1024)}`
  assert.ok(Buffer.byteLength(huge) > ref.MAX_BUNDLE_TEXT_BYTES)
  const started = performance.now()
  const r = await ref.verifyProfileBundle(huge)
  assert.equal(r.integrity, 'MALFORMED'); assert.equal(r.reasonCode, 'bundle.too-large')
  assert.ok(performance.now() - started < 500, 'refused without parsing')
})
...
test('routing: a BARE scan attestation (no attestationType) is routed to the legacy verifier; a legacy payload inside a bundle is malformed', async () => {
  ...
  const bare = await ref.verifyProfileBundle(structuredClone(legacy))
  assert.equal(bare.route, 'LEGACY_SCAN_ATTESTATION'); assert.notEqual(bare.integrity, 'VALID')
  const wrapped = await ref.verifyProfileBundle({ bundleVersion: '1.0.0', attestation: structuredClone(legacy), assessment: {} })
  assert.equal(wrapped.route, 'PROFILE_ASSESSMENT'); assert.equal(wrapped.integrity, 'MALFORMED')
  assert.equal((await ref.verifyProfileBundle(signedBundle('B1'))).route, 'PROFILE_ASSESSMENT')
  const noVersion = signedBundle('B1'); delete noVersion.bundleVersion
  assert.equal((await ref.verifyProfileBundle(noVersion)).integrity, 'MALFORMED')
})
...
test('cross-type: a profile payload without its type is malformed inside a bundle, and a legacy payload given a profile type is never accepted', async () => {
  ...
  const r = await ref.verifyProfileBundle(b)
  ...
  const d = await ref.verifyProfileBundle(dressed)
...
test('LIVE closed-registry proof with a throwaway in-memory key: an INHERITED registry entry is not a registered profile', async () => {
  ...
  const inheritedOnly = Object.create({ inherited: { framework: 'OWASP_AGENTIC_SKILLS_TOP_10', profileVersions: ['1.0.0-alpha.1'] } })
  const r = await ref.verifyProfileBundle(b, { supportedProfiles: inheritedOnly })
  assert.equal(r.integrity, 'VALID')
  assert.equal(r.interpretation, 'UNSUPPORTED_PROFILE', 'an inherited entry is not in the closed registry')
  const own = await ref.verifyProfileBundle(b, { supportedProfiles: { inherited: { framework: 'OWASP_AGENTIC_SKILLS_TOP_10', profileVersions: ['1.0.0-alpha.1'] } } })
