// ONE-SHOT vector generator for VECTOR SET v2. TEST-ONLY. Not part of any product build.
//
//   node conformance/tools/generate-vectors.mjs --new-vector-set
//
...
//
// v2 differs from v1 in: profileId in the assessment; bundleVersion on the bundle; low-S signatures for the profile type (and
// deliberate low-S AND high-S legacy vectors); the signed-content numeric profile (integers only); key STATES instead of key
// trust; key-set metadata; the closed profile registry; canonical JWK coordinates; the domain registry; and a hardened matrix.
...
import { NUMBER_PROFILE_ID } from '../reference/numberProfile.mjs'
import * as ref from '../reference/profileAttestation.mjs'
import { StrictJsonError, parseStrictJson } from '../reference/strictJson.mjs'
import { applyPatch, assertSubset, resolveSource } from '../test/helpers.mjs'
...
if (existsSync(path.join(OUT, 'manifest.v2.json')) && !process.argv.includes('--overwrite')) {
...
const head = (extra = {}) => ({ vectorSet: VECTOR_SET, version: VERSION, supersedes: 'vector set v1 (v1 signatures were not low-S; v1 is replaced, not patched)', note: NOTE, ...extra })
...
const otherAssessment = () => { const a = baseAssessment(); a.evidence.pop(); a.unmappedEvidenceIds = ['ev-0001']; a.notes.push('A different synthetic assessment.'); return a }
...
const UNSUP = interpretation => ({ integrity: 'VALID', interpretation, reasonCode: 'ok' })
{
  ...
  const fw = baseAssessment(); fw.profile.framework = 'OTHER_FRAMEWORK'
  await addSigned('S_framework-mismatch', 'A known profile id whose framework is not the registered one (payload and assessment agree with each other).', fw)
  addCase('interpretation.framework-not-registered', 'The id is known but the framework is not the registered one: UNSUPPORTED_PROFILE.', { signed: 'S_framework-mismatch' }, UNSUP('UNSUPPORTED_PROFILE'))
  ...
  await addSigned('S_only-ast03', 'A validly signed bundle from a profile version that implemented only AST03 (used for the downgrade policy vector).', down)
}
...
textMal('number-underflow', 'a nonzero literal that a double would silently turn into zero (1e-400): refused', insertAtBundleAssessment('"tiny":1e-400,'), 'JSON_NUMBER_UNDERFLOW')
textMal('number-underflow-huge-exponent', '1e-99999999999 is nonzero: refused without doing any arithmetic on the exponent', insertAtBundleAssessment('"tiny":1e-99999999999,'), 'JSON_NUMBER_UNDERFLOW')
...
textMal('number-very-long-fraction', 'a fraction with 450 leading zeros is a nonzero value that underflows: refused', insertAtBundleAssessment(`"tiny":0.${'0'.repeat(450)}1,`), 'JSON_NUMBER_UNDERFLOW')
...
invalidSet('entry-purpose-empty', 'A key set entry whose purpose is present but EMPTY: purpose must be explicit and non-empty. (Coverage vector added after the first v2 generation and before the v2 freeze.)', 'badPurposeEmpty', 'keySet.entry.purpose')
invalidSet('entry-notBefore-not-canonical', 'A key set entry whose notBefore is not a canonical timestamp (2026-01-01 instead of 2026-01-01T00:00:00.000Z). (Coverage vector added after the first v2 generation and before the v2 freeze.)', 'badNotBefore', 'keySet.entry.notBefore')
...
policy('downgrade', 'DOWNGRADE DEFENCE: an attestation from when only AST03 existed cannot satisfy a verifier that requires all three controls, without parsing the assessment.', { signed: 'S_only-ast03' }, 'active', { requiredControls: ALL3 }, { integrity: 'VALID', policy: NOT('CONTROLS_NOT_COVERED') })
...
const optsFor = c => ({ ...(c.keySet ? { keySet: keySets[c.keySet] } : {}), ...(c.policy ? { policy: c.policy } : {}), ...(c.options ?? {}) })
for (const c of cases) {
  const actual = await ref.verifyProfileBundle(resolveSource(vectorForCheck, c.source), optsFor(c))
...
write('bundles.v2.json', head({
  constants: { issuer: ISSUER, profileId: PROFILE_ID, keyPurpose: ref.PROFILE_KEY_PURPOSE, issuedAt: ISSUED_AT, bundleVersion: ref.BUNDLE_VERSION, keySetVersion: ref.KEY_SET_VERSION, maxBundleTextBytes: ref.MAX_BUNDLE_TEXT_BYTES },
  ...
  crossChecks: ref.CROSS_CHECKS.map(c => c.binding),
...
write('assessment-digest.v2.json', head({ formula: 'avassess-sha256: + hex(SHA-256(UTF8("agentverify-assessment-digest/v1\\n") || UTF8(JCS(assessment)))), JCS under the signed-content numeric profile', cases: digestCases }))
...
// ── avpkg.v2.json (package identity) ─────────────────────────────────────────────────────────
...
write('avpkg.v2.json', head({ formula: '"avpkg-sha256:" + hex(SHA-256(UTF8("agentverify-skill-package-digest/v1\\n") || for each file sorted by UTF-8 path bytes: UTF8(len(path)) ":" path "\\n" UTF8(len(content)) ":" content "\\n"))  (lengths are decimal byte counts)', cases: avpkgCases }))

// ── domains.v2.json: the registry, frozen ─────────────────────────────────────────────────────
...
const rejectedTexts = [
  ...
  ['underflow-to-zero', '[1e-400]', 'JSON_NUMBER_UNDERFLOW'], ['underflow-huge-exponent', '[1e-99999999999]', 'JSON_NUMBER_UNDERFLOW'], ['underflow-long-fraction', `[0.${'0'.repeat(450)}1]`, 'JSON_NUMBER_UNDERFLOW'],
  ...
  ['dominant-exponent-positive', `[1e${'9'.repeat(40)}]`, 'JSON_NUMBER_UNSAFE_INTEGER'], ['dominant-exponent-negative', `[1e-${'9'.repeat(40)}]`, 'JSON_NUMBER_UNDERFLOW'],
...
// ── legacy-attestation.v2.json: frozen scan attestations, verified by scanner, web port and CI stub ──
//
...
legacyCases.push({ name: 'cross-type.profile-attestation-is-malformed', description: 'The legacy verifier given a profile attestation fails closed: the required scan fields are absent. (The attestation object is taken from bundles.v2.json, B1.)', fromBundle: 'B1', expected: { status: 'MALFORMED' } })
...
write('legacy-attestation.v2.json', head({
...
const manifest = {}
for (const f of readdirSync(OUT).filter(f => f.endsWith('.v2.json') && f !== 'manifest.v2.json').sort()) manifest[f] = sha256Hex(readFileSync(path.join(OUT, f)))
write('manifest.v2.json', { note: 'SHA-256 of every vector file in vector set v2. If a vector changes, this changes, and the change is visible in review.', files: manifest })
