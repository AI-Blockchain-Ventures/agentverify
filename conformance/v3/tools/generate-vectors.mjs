// ONE-SHOT vector generator for VECTOR SET v3. TEST-ONLY. Not part of any product build.
//
//   node conformance/v3/tools/generate-vectors.mjs --new-vector-set
//
...
//
// v3 supersedes v2 (v2 is FROZEN and untouched; its manifest hash remains the reviewed historical artifact). v3 differs from v2 in:
// an O(n) exact-value numeric classifier; strict fail-closed verifier policy; SUPPORTED defined by an exact assessment schema; a
// closed profile registry that pins the upstream commit, licence, controls and schema; one shared depth model; BYTES as the
// normative verifier input (strict UTF-8, no BOM); one strict timestamp parser; a payload-field classification registry;
// and the extra vectors the second review asked for.
...
if (existsSync(path.join(OUT, 'manifest.v3.json')) && !process.argv.includes('--overwrite')) {
...
const head = (extra = {}) => ({ vectorSet: VECTOR_SET, version: VERSION, supersedes: 'vector set v2 (frozen, unchanged; v3 is a NEW set, not a patch of v2)', note: NOTE, ...extra })
...
const otherAssessment = () => { const a = baseAssessment(); a.evidence[2].summary = 'A different synthetic context item.'; a.unmappedEvidenceIds = ['ev-0003']; a.notes.push('A different synthetic assessment.'); return a }
...
// ══ VECTOR SET v3 ADDITIONS (bundle-level) ═══════════════════════════════════════════════════
//
...
// The assessment-schema mutation table. It is used twice: here (a sample, signed, end to end) and in assessment-schema.v3.json (every case, direct).
...
const DEFINITION_CASES = [
  ...
  ['implemented-controls-subset', [rep('/profile/implementedControls', ['AST02', 'AST03']), rmv('/controls/2')], 'profile.implemented-controls'],
...
write('assessment-digest.v3.json', head({ formula: 'avassess-sha256: + hex(SHA-256(UTF8("agentverify-assessment-digest/v1\\n") || UTF8(JCS(assessment)))), JCS under the signed-content numeric profile', cases: digestCases }))
...
// ── avpkg.v3.json (package identity) ─────────────────────────────────────────────────────────
...
write('avpkg.v3.json', head({ formula: '"avpkg-sha256:" + hex(SHA-256(UTF8("agentverify-skill-package-digest/v1\\n") || for each file sorted by UTF-8 path bytes: UTF8(len(path)) ":" path "\\n" UTF8(len(content)) ":" content "\\n"))  (lengths are decimal byte counts)', cases: avpkgCases }))

// ── domains.v3.json: the registry, frozen ─────────────────────────────────────────────────────
...
write('jcs.v3.json', head({
  ...
  adversarialNumbers: { note: 'Long runs of zeros in the MIDDLE of integer, fractional and exponent forms. The token is rebuilt by adversarialToken(kind, N) (conformance/v3/test/helpers.mjs); expected is the exact value or the refusal. The v2 classifier was quadratic on these.', sizes: ADVERSARIAL_SIZES, cases: adversarialCases },
...
// ── legacy-attestation.v3.json: frozen scan attestations, verified by scanner, web port and CI stub ──
//
...
legacyCases.push({ name: 'cross-type.profile-attestation-is-malformed', description: 'The legacy verifier given a profile attestation fails closed: the required scan fields are absent. (The attestation object is taken from bundles.v3.json, B1.)', fromBundle: 'B1', expected: { status: 'MALFORMED' } })
...
write('legacy-attestation.v3.json', head({
...
// ══ VECTOR SET v3: NEW VECTOR FILES ═══════════════════════════════════════════════════════════

// ── assessment-schema.v3.json ─────────────────────────────────────────────────────────────────
write('assessment-schema.v3.json', head({
...
write('profile-registry.v3.json', head({
...
// ── admission.v3.json: the signer-side admission predicate (verification logic; no signing) ────
{
  const ADM = []
...
const manifest = {}
for (const f of readdirSync(OUT).filter(f => f.endsWith('.v3.json') && f !== 'manifest.v3.json').sort()) manifest[f] = sha256Hex(readFileSync(path.join(OUT, f)))
write('manifest.v3.json', { note: 'SHA-256 of every vector file in vector set v3. If a vector changes, this changes, and the change is visible in review.', files: manifest })
