...
//   * (v4) EVERY evidence item is accounted for EXACTLY ONE of two ways: referenced by at least one check's
//     supportingEvidenceIds, or listed in unmappedEvidenceIds. Never both, never neither. A "gap" evidence item that a signer
//     (or a compromised signing key) left out of every check AND out of unmappedEvidenceIds used to still be SUPPORTED; a
//     third independent review found this exact hole (a finding could be orphaned from the control-level rollup while the
//     assessment as a whole still validated), so it is now closed as one more schema rule, not a redesign of the evidence
//     model. Many-to-many references remain valid: one evidence item may back several checks, and one check may cite several.
//
...
export const ASSESSMENT_SCHEMA_PROBLEM_CODES = Object.freeze([
  ...
  'assessment.schema.evidence-mapping-conflict',
  'assessment.schema.evidence-unaccounted',
...
export function assessmentSchemaProblem(a) {
  ...
  // (v4) every evidence id any check actually cites, across every control: the "referenced" half of the completeness rule.
  // Many-to-many is fine, so this is a Set (membership only), not a count.
  ...
  // (v4) EVIDENCE-ACCOUNTING COMPLETENESS: allEvidenceIds === referencedEvidenceIds UNION unmappedEvidenceIds, and that union is
  // disjoint. Checked as two separate rules, in this order, so a vector can isolate which one it is proving:
  //   1. no id is claimed unmapped while a check also cites it (a self-contradiction: "no check covers this" and "a check covers
  //      this" cannot both be true of the same evidence item)
  //   2. no evidence id is silently absent from BOTH sides (an id that is neither cited by any check nor declared unmapped is an
  //      orphaned finding: nothing in the control-level rollup reflects it, however security-relevant it is)
  // A many-to-many reference pattern (one evidence item backing several checks, one check citing several items) is unaffected:
  // this is a set-membership rule, not a cardinality rule.
