...
const SIGNING = loadVector('signing.v4.json')
const DIGESTS = loadVector('assessment-digest.v4.json')
const KEYIDS = loadVector('keyid.v4.json')
const AVPKG = loadVector('avpkg.v4.json')
...
test('there is no TRUSTED key state anywhere: not in the reference, not in any vector', () => {
  ...
  const vector = JSON.parse(readFileSync(path.join(here, '..', 'vectors', 'bundles.v4.json'), 'utf8'))
