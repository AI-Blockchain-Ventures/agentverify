// MUTATION TESTING of the conformance reference (vector set v4). TEST-ONLY. Slow (several minutes); not part of the gate.
//
//   node conformance/v4/tools/mutation-check.mjs [--only "mutation name|another name"]
//
// It copies conformance/v4 to a scratch directory, applies ONE small mutation at a time to the REFERENCE (the repository is never
...
//
// v3 added mutation classes for: the O(n) numeric classifier, the shared depth model, the bytes decoder, the strict timestamp parser,
...
// field classification, and the cross-check registry. v4 adds mutations for exactly its one code change: evidence-accounting
// completeness in the exact assessment schema (the third review's blocker #1; see conformance/v4/README.md).
...
const scratch = mkdtempSync(path.join(tmpdir(), 'agentverify-mutation-v4-'))
const copy = path.join(scratch, 'v4')
...
// ── v4: evidence-accounting completeness (the third review's blocker #1) ────────────────────
m('schema: referencedEvidenceIds never collected (nothing is ever "referenced")', S, 'for (const id of k.supportingEvidenceIds) referencedEvidenceIds.add(id)', '')
m('schema: evidence-mapping-conflict check removed (referenced-and-unmapped admitted)', S, "for (const id of a.unmappedEvidenceIds) if (referencedEvidenceIds.has(id)) return 'assessment.schema.evidence-mapping-conflict'", '')
m('schema: evidence-unaccounted check removed (orphaned evidence admitted)', S, "for (const id of evidenceIds) if (!referencedEvidenceIds.has(id) && !unmappedEvidenceIdSet.has(id)) return 'assessment.schema.evidence-unaccounted'", '')
m('schema: evidence-unaccounted check reads the wrong set (checks unmapped membership twice, never referenced)', S, 'if (!referencedEvidenceIds.has(id) && !unmappedEvidenceIdSet.has(id))', 'if (!unmappedEvidenceIdSet.has(id) && !unmappedEvidenceIdSet.has(id))')
m('schema: conflict check inverted (requires BOTH sides empty instead of any overlap)', S, 'if (referencedEvidenceIds.has(id)) return', 'if (referencedEvidenceIds.size === 0) return')
