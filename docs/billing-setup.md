...
Agent Verify v1.3 pricing UI is ready for Free, Pro, Team, and Enterprise packaging. Stripe checkout is enabled only for test mode until live billing receives explicit approval.
...
- `/pricing` shows final packaging and routes Pro to the Worker checkout route for logged-in users during Stripe test-mode validation.
...
- `NEXT_PUBLIC_BILLING_CHECKOUT_ENABLED`: set to `true` only after secure backend checkout and entitlement enforcement are deployed.

Do not commit Stripe secret keys, webhook signing secrets, API keys, Firebase service-account credentials, or restricted credentials.
...
Worker routes implemented for test-mode activation:
...
- `BILLING_ENABLED`: `true` only after test-mode checkout and webhook verification are ready.
...
- Create a D1 database: `wrangler d1 create agentverify-billing-test`
- Apply schema locally for review: `wrangler d1 execute agentverify-billing-test --local --file workers/api/schema/billing.sql`
- Apply schema to the test D1 database: `wrangler d1 execute agentverify-billing-test --remote --file workers/api/schema/billing.sql`
...
Required production test-mode redirect values:
...
- `STRIPE_PRO_PRICE_ID=price_1Tqc85RqPRlpuX8nAWvN5Jk5`

Temporary localhost test-mode redirect values:
...
## Safe Worker Test-Mode Deployment Steps

Do not use live Stripe keys for this sequence.
...
2. Configure non-secret Worker variables:
   - `BILLING_ENABLED=false` initially
   - `FIREBASE_PROJECT_ID`
...
7. Set `BILLING_ENABLED=true` only for Stripe test mode.
...
11. Create the Stripe test-mode webhook endpoint only after the deployed route responds correctly.
12. Send a signed Stripe test event and confirm subscription state is written by the Worker to D1 `subscriptions`.
...
Firestore remains used for reports for now. Firestore report privacy and report rules are separate from billing storage and are not required for Stripe test-mode billing state.
...
## Stripe Test-Mode Product Setup

- In Stripe test mode, create product `Agent Verify Pro`.
- Add a recurring monthly price of `$19.99 USD`.
- Copy only the test-mode price ID into the `STRIPE_PRO_PRICE_ID` Worker secret binding.
...
- Keep `BILLING_ENABLED` disabled until checkout and webhook tests pass end-to-end, then set it only for Stripe test mode.

Test-mode versus live-mode checklist:

- Test-mode Stripe keys and price IDs must only be used with test-mode webhook endpoints.
- Live-mode keys and price IDs must only be configured after a separate approval step.
- Never mix test-mode `STRIPE_PRO_PRICE_ID` with a live secret key.
- Never commit any Stripe key, webhook secret, Firebase key, token, or service-account credential.
- Live billing requires explicit approval and deployment after all backend entitlement checks are complete.

## Stripe Event Handling

Handle these events before charging customers in production:
...
Safe test-row repair after the early `status=complete` bug:
...
This is only for the known Stripe test-mode row. Prefer replaying or resending the real `customer.subscription.created` or `customer.subscription.updated` webhook after deploying the fix so D1 stores the real Stripe subscription status and period end.

Safe test-row repair when `status=active` but `current_period_end` is null:

1. Deploy the Worker fix to the test-mode Worker.
2. In Stripe test mode, open subscription `sub_1TrgjNRqPRlpuX8nUKt1GObi` and resend a fresh `customer.subscription.updated` event to the Worker webhook, or update a harmless subscription metadata value to cause Stripe to send `customer.subscription.updated`.
...
Do not use the manual update for production data. For production, rely on signed Stripe webhooks and subscription retrieval.
...
- `apps/web/src/lib/billing.ts`: call backend billing routes instead of contact fallback after checkout is live.
...
## Local Test Plan Without Live Stripe Credentials
...
- Verify UI handles checkout unavailable states without implying live billing.
...
- Create a test Pro price for $19.99/month.
- Configure webhook endpoint for the Worker test URL.
