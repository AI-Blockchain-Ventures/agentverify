...
const findingDefinitions = {
  missingSignature: (): Finding => ({
    ...
    recommendedFix: 'Implement RSA or HMAC signature verification on every execution request using the A2SPA protocol.',
    a2spaFix: 'Use A2SPA signPayload() to sign every agent request. Verify with verifySignature() before execution.',
  }),
  missingNonce: (): Finding => ({
    ...
    a2spaFix: 'Use A2SPA generateNonce() and validateNonce() on every request.',
  }),
  missingTimestamp: (): Finding => ({
    ...
    a2spaFix: 'Use A2SPA addTimestamp() and enforceExpiry() with a 300 second window.',
  }),
  missingFailClosed: (): Finding => ({
    ...
    a2spaFix: 'Use A2SPA enforceFailClosed() — execution is blocked by default on any check failure.',
  }),
  missingScope: (): Finding => ({
    ...
    a2spaFix: 'Use A2SPA defineScope() to declare minimum required permissions. The protocol rejects out-of-scope actions.',
  }),
...
function scan(input: ScanInput): ScanResult {
  ...
  const categoryScores: CategoryScore[] = [
    { category: 'A', label: 'A2SPA Protocol Compliance', score: Math.max(0, 50 - catAFindings.length * 10), maxScore: 50, findingCount: catAFindings.length },
    { category: 'B', label: 'General Agent Security', score: Math.max(0, 50 - catBFindings.length * 10), maxScore: 50, findingCount: catBFindings.length },
  ]
